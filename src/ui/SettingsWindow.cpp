 ...
 *
 * Settings are stored in Ideam/ideam.settings file in default user settings
 * directory.
 *
 * Features:
 *
	...
	, fAppliedModifications(0)
{
	...
	fWindowSettingsFile = new TPreferences(IdeamNames::kSettingsFileName,
...
SettingsWindow::_ApplyOrphans()
{
	...
	if (orphans > 0) {
		...
		fWindowSettingsFile->SetInt64("last_used", real_time_clock());
		// Reset app version
		fWindowSettingsFile->SetBString("app_version", IdeamNames::GetVersionInfo());
	}
	_UpdateText();
	_UpdateTrailing();
}
